...
def test_setup_custom_logging_file_handler_exception(
    ...
    # Check for any error message (platform-independent)
    assert (
        "Permission denied" in caplog.text
        or "cannot create log file" in caplog.text
        or "Read-only file system" in caplog.text
        or "Errno" in caplog.text
    )
