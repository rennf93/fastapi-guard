...
- **Added** - **`StarletteGuardRequest` implements guard-core 3.12.0's `BoundedBodyReader` capability.** A request without a usable `Content-Length`, which is every `Transfer-Encoding: chunked` request, previously reached penetration detection as an unreadable body and was waved through unscanned; with guard-core older than 3.12.0 the same request instead triggered the unbounded buffering that GHSA-c2r5-9jw9-m8q5 closed. `read_body_prefix` reads at most the requested prefix plus at most one in-flight chunk, then installs a replaying receive so the downstream endpoint still observes the byte-identical full body. Both properties are locked in by end-to-end tests through a real FastAPI app: an attack in a chunked body is blocked with `400 Suspicious activity detected`, and a benign chunked body arrives at the endpoint whole.
