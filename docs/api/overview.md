...
For IP and country access control, this priority applies per-aspect rather than as a blanket override: the global IP whitelist/blacklist and country rules always run on decorated routes. A route-level allow rule (`ip_whitelist` or `whitelist_countries`) suppresses the corresponding global gate. A route-level deny rule (`ip_blacklist` or `blocked_countries`) is additive and does not disable the global rules.
